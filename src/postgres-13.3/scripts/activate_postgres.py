                ...
                if (username,) in list_users:
                    ...
                    for x in role:
                        if x == 'readonly':
                            ...
                            cursor1.execute(f"GRANT SELECT ON ALL TABLES IN SCHEMA public TO {username};")
                            cursor1.execute(f"ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO {username};")
                        else:
                            userPermission = "alter user " + username + " with " + x + ";"
                            cursor1.execute(userPermission)
                            logging.info("'{}' User role updated to {}".format(username, x))
        else:
        ...
        if 'databases' in words:
            ...
            for db in databases:
                ...
                if os.path.exists(location):
                    logging.warning("'{}' Tablespace location already exists".format(location))
                else:
